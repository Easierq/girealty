...
export const BedroomForm = ({ initialData, listingId }: LatFormProps) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      bedroom: initialData?.bedroom || undefined,
    },
  });
      ...
      {!isEditing && (
        <p
          className={cn(
            ...
            !initialData.bedroom && "text-slate-500 italic"
          )}
          onClick={toggleEdit}
        >
          {initialData.bedroom || "No bedroom"}
        </p>
      )}
      {isEditing && (
        <Form {...form}>
